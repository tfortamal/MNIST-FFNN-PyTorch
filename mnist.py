...
# instance of network
model = MyMnistModel().to(device=device)

xs, ys = trainDs[0:4]


# making first prediction and calculating the loss
pred = model(xs.to(device=device))
...
print("----------------------------\n")
epoch_data, loss_data = train_model(train_dl, model, epoch, lr= learningRate)
...
finalPred = model(xs).argmax(axis=1)
